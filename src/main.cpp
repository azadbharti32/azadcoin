...
#if defined(NDEBUG)
# error "Azadcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x62a10255814bb797efbdc6c94c97bf834e5e8221de2628b7599fe074ba7d305f");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Azadcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Azadcoin Signed Message:\n";
...
{
    // Azadcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Azadcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10*30; // Azadcoin: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // Azadcoin: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Azadcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Azadcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x62a10255814bb797efbdc6c94c97bf834e5e8221de2628b7599fe074ba7d305f");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is azadcoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1524470328;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1984361;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Azadcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AzadcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AzadcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AzadcoinMiner(CWallet *pwallet)
{
    printf("AzadcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("azadcoin-miner");
    ...
    try { loop {
        ...
        printf("Running AzadcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AzadcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AzadcoinMiner, pwallet));
}
